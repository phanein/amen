% Bryan Perozzi
function [weighted_score, raw_score, internal_edges, external_edges, internal_weights, boundary_weights] = ...
    amen_objective (A, X_Transpose, community, weights, degrees, M, use_community_features, continuous_features)
% 
% input: 
...
%     I_min = epsilon;
    ...
    if continuous_features == 0
        % discrete feature code, uses kronecker delta for similarity
    ...
    %     NotAllXors = ~AllXors;
        ...
        % continuous feature code, uses dot product for similarity
        ...
        if numel(B_internal) > 0
            ...
            for a = 1:numel(B_internal)
                ...
                surprisingness = 1 - min(1, (k_(i) * degrees(b)) / (2 * M));         
                similarity = X_(i,:) .* BX_(a,:);  % dot product similarity            
                boundary_weights = boundary_weights + surprisingness * similarity.';
            end
        end         
    ...
    if no_weights == 1
        weighted_score = NaN;    % no weights, so weighted score doesn't exist
    else
        weighted_score = dot(original_weights, data_score);    % weighted score from amen objective
    end
    
    raw_score = data_score;  % raw score is unweighted values we compute from the data
end
